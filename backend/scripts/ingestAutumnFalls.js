...
(async()=>{
  ...
  for(const F of FILES){
    const rf=await db.get("SELECT id, original_name FROM request_files WHERE filename=?",[F.disk]);
    if(!rf){ console.log('SKIP (no request_file):',F.disk); continue; }
    ...
    await db.run("INSERT INTO fulfilled_records (id, source_file_id, output_file_id, title, summary, record_type_id, department_id, status, page_count, released_by, released_at) VALUES (?,?,?,?,?,?,?,'released',1,'Library Seed',datetime('now'))",
      [frId, rf.id, rf.id, crude, 'Uploaded record '+rf.original_name, F.rt, F.dept]);
    await rme.enrichFulfilledMeta(frId);
    const a=await db.get("SELECT title, event_date FROM fulfilled_records WHERE id=?",[frId]);
    console.log('INGESTED ->', a.title, '| date', a.event_date);
  }

...
(async function () {
  ...
  ok('A3 create category: SUPERVISOR 403 (oversight role, not Workflow & Taxonomy)', gated(await call(T.super, 'POST', '/taxonomy/categories', { name: 'x', code: 'x' })));
  ...
  ok('A7 create record type: supervisor 403', gated(await call(T.super, 'POST', '/taxonomy/record-types', {})));
  ...
  ok('A13 set routing: supervisor 403', gated(await call(T.super, 'PATCH', '/taxonomy/record-types/' + NX + '/routing', {})));
  ...
  ok('A18 apply a variant proposal (inserts a draft): supervisor 403', gated(await call(T.super, 'POST', '/taxonomy/record-types/' + NX + '/variants', { name: 'x' })));
  ...
  ok('A21 discover-scan (inserts drafts): supervisor 403', gated(await call(T.super, 'POST', '/taxonomy/discover-scan', { repository_id: NX })));

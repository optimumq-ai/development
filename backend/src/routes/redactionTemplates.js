...
// Build a simple layout fingerprint from the sample document (placeholder for a richer AI/visual one).
async function buildFingerprint(fileId) {
  ...
  var pages = await all('SELECT page_no, text FROM document_pages WHERE file_id = ? ORDER BY page_no', [fileId]);
  var name = file ? (file.original_name || file.filename) : 'document';
  var snippet = '';
  if (pages[0] && pages[0].text) snippet = pages[0].text.replace(/\s+/g, ' ').trim().slice(0, 180);
  return 'Defined from ' + name + ' (' + pages.length + ' page(s)).' + (snippet ? ' Page 1 begins: ' + snippet : '');
}
...
router.post('/:id/apply', requireAuth, async function(req, res) {
  ...
  try {
    ...
    await run('INSERT INTO redaction_jobs (id, file_id, request_id, jurisdiction_id, status, created_by) VALUES (?,?,?,?,?,?)',
      [jobId, fileId, file.request_id, t.record_type_id ? await activeJurisdiction() : await activeJurisdiction(), 'draft', req.user.sub]);
    for (var i = 0; i < zones.length; i++) {
      var z = zones[i];
      await run('INSERT INTO redaction_zones (id, job_id, file_id, page_no, x, y, w, h, rule_id, note, zone_type, created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
        [uuidv4(), jobId, fileId, z.page_no || 1, z.x, z.y, z.w, z.h, z.rule_id || null, z.label || null, 'template', req.user.sub]);
    }
    var result = await redactionApply.applyRedaction(jobId, req.user.name || 'Mass Redaction');
    res.json(Object.assign({ success: true, templateId: t.id, jobId: jobId }, result));
  } catch (e) {

...
router.post('/request/:requestId/notice/send', requireAuth, async function (req, res) {
  try {
    ...
    if (ok) await run("UPDATE tasks SET status = 'done', updated_at = datetime('now') WHERE request_id = ? AND type = 'estimate' AND status IN ('open','assigned','in_progress','returned')", [req.params.requestId]);

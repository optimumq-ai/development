...
router.post('/:id/approve', requireAuth, requireAuthority('financial_approval'), async function (req, res) {
  try {
    var o = await get('SELECT * FROM objections WHERE id = ?', [req.params.id]);
    if (!o) return res.status(404).json({ error: 'Objection not found.' });
...
router.get('/pending-approval', requireAuth, requireAuthority('financial_approval'), async function (req, res) {
  try {

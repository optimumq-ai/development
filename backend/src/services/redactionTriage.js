...
async function cancelRedactionTask(requestId) {
  await run("UPDATE tasks SET status = 'cancelled', updated_at = datetime('now') WHERE request_id = ? AND type IN ('redaction','legal_redaction') AND status IN ('open','assigned','in_progress','returned','awaiting_review')", [requestId]);
}

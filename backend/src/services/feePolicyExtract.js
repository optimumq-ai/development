...
function buildPrompt(text, context) {
  ...
  return ''
    ...
    + '  "requestRules": {"freePageAllowance": null, "freeLaborHours": null, "deMinimis": null, "minFee": null, "maxFee": null, "deposit": {"threshold": null, "percent": null}, "estimateNotifyThreshold": null, "surchargePct": null},\n'
    + '  "purposeOverrides": null,\n'
    + '  "estimatePolicy": {"requesterResponseDays": null, "revisionNotifyPercent": null, "estimateValidityDays": null}\n'
    + '}\n'
    + 'Units: labor rate is $/hour; duplication is $/page; media is $/item; for GRADUATED page rates set duplication.<type>.tiers=[{"upTo":N,"rate":R}, ...] in order (omit upTo on the final band for unlimited), e.g. first 50 pages free then 0.15 -> [{"upTo":50,"rate":0},{"rate":0.15}]; otherwise use a flat rate. handling is $ flat. av.perRecording is $ per audio/video recording and av.perMinute is $ per minute of recording (e.g. police body-cam); av.freeMinutes is a minute count. '
    + 'labor.increment is one of 0 (bill actual time), 0.25, 0.5, 1 (hours). rounding is "up", "down", or "nearest". labor.overheadPct is a percent surcharge on billable labor (e.g. 20 for a Texas +20% overhead). For EACH labor driver: set billable=false when the jurisdiction forbids charging for that labor at all (e.g. California, New York, Ohio labor); OR set billableWhen={"mode":"all_or_nothing","trigger":"pages" or "hours","threshold":N} when labor is chargeable only once a threshold is exceeded (e.g. Texas: no labor until total pages exceed 50 -> trigger "pages", threshold 50; New York preparation only beyond 2 hours -> trigger "hours", threshold 2). Leave billable=true and billableWhen=null when labor is normally chargeable. '
    + 'deposit.percent is a number like 50 for 50%. requestRules thresholds are dollar amounts. '
    + 'freePageAllowance is a page count; freeLaborHours is hours. estimatePolicy.requesterResponseDays = business days the requester has to accept an estimate before it lapses; revisionNotifyPercent = re-notify the requester if the cost changes by more than this percent; estimateValidityDays = days an estimate stays valid. requestRules.surchargePct is a percent surcharge on the whole subtotal. If the jurisdiction charges COMMERCIAL-purpose requests differently (e.g. Arizona, Illinois), set purposeOverrides to {"commercial": { ...only the fields that DIFFER for commercial... }} - e.g. {"commercial": {"requestRules": {"surchargePct": 15}, "labor": {"search": {"billable": true}, "review": {"billable": true}, "programming": {"billable": true}}}} to charge labor + a surcharge for commercial requests; For jurisdictions where on-site INSPECTION carries no fee (e.g. Washington, Ohio), also set purposeOverrides.inspection={"labor":{"search":{"billable":false},"review":{"billable":false},"programming":{"billable":false}}}. Otherwise leave purposeOverrides null.\n\n'

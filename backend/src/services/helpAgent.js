...
var APP_CONTEXT = [
  ...
  '- Setup: Administration → Settings and Configuration is the setup hub — every setup item as a row with its counted state, opening its own screen; the Set Up Guide tab shows the same items as a plan (steps and dependencies) before going live.',
  '- Reports: analytics and reporting — ARIA Reports and AI Reporting as tabs of one screen.',
  '- Administration: two tabs — Settings and Configuration (the hub) and Set Up Guide. Every configuration screen is reached from a hub row: Agency name/address (Start here), Fee rules (/setup/fee-law: what the law sets, city decisions, fee policy document, test an estimate), Request rules (/setup/request-rules: clarification, exemptions, eligibility, deadlines, intake), Redaction Rules Library, Update Configuration (proposed changes to review), Record Sources and Connectors, AI configuration (service keys, deployment model, AI touchpoints, portal security), Email configuration, User Authentication Setup, Taxonomy, Workflow Rules, Process Map, Task Time Budgets, Task Processing Time Capture, System Notifications, Video Redaction Options, Portal Agent Rules, User Types.',
  ...
  '- Taxonomy (/setup/taxonomy): the catalog of record types, their release posture, owning department and estimate calibration, used for AI classification and routing.',
  '- Workflow Rules (/setup/workflow-rules) and Process Map (/setup/process-map): the configurable request routing rules and the visual map of every decision a request passes through.',
  '- Record Sources and Connectors (/setup/record-sources): connected record systems (connectors) and repositories the platform searches or imports from. Integration is by direct connection where a system has an API, or by scheduled export/drop-folder where it does not.',
  '- Redaction Rules Library (/setup/redaction-rules): the library of legal exemptions/rules used to justify redactions.',
  ...
  '- Fee rules (/setup/fee-law) sets the fee schedule; Cash Drawer records payments.',
  '- Tickler: scheduled reminders/automation. Update Configuration (/setup/update-configuration) / Jurisdiction Configuration: jurisdiction-specific rules (statutes, deadlines, fee structures) - jurisdiction is configuration, not a separate product. Configuration (admins only): system settings.',
  '',

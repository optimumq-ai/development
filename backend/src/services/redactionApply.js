...
async function applyRedaction(jobId, actor) {
  ...
  try {
    ...
    // The verification anchor (§2.2): hash the exact bytes just written, before anything can touch them.
    var contentSha = require('./fileHash').ofBuffer(bytes);
    // ON CONFLICT: the triage sweep and a manual apply can interleave inside one second (smoke run 5) —

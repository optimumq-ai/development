...
N({id:'assign-workload',phase:'routing',label:'Is Auto Load Balancing on for the owning team?',decider:'code',status:'partial',criteria:['Reads the Auto Load Balancing setting for the team that owns the task','Workload pool = team members who hold the relevant role (Record Search or Redaction)'],outcomes:[{label:'On -> assign to the member with the smallest current workload',note:'Auto-balances open task load across qualified team members.'},{label:'Off -> offer to the team for claim',note:'Any eligible team member can claim the task from the queue.'}],automatedBy:'turn on Auto Load Balancing for the team (governs both Record Search and Redaction task assignment)'});
...
"assign-workload":"Once a task is ready to be worked, this decides which person on the team gets it. If the owning team has Auto Load Balancing turned on, the system assigns the task to the qualified member with the smallest current workload - looking only at people who hold the relevant role (Record Search for a search task, Redaction for a redaction task). If it is off, the task is offered to the whole team and any eligible member can claim it.",

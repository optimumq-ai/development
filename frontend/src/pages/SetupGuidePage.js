...
var STATE = {
  ready:           { label: 'Ready',           bg: '#DCFCE7', bd: '#16A34A', dot: '#16A34A' },
  in_progress:     { label: 'In progress',     bg: '#FEF3C7', bd: '#D97706', dot: '#D97706' },
  not_started:     { label: 'Not started',     bg: 'white',   bd: '#C7D0D8', dot: '#C7D0D8', dashed: true },
  waiting:         { label: 'Waiting',         bg: '#EDE9FE', bd: '#7C3AED', dot: '#7C3AED' },
  needs_attention: { label: 'Needs attention', bg: '#FEE2E2', bd: '#DC2626', dot: '#DC2626' },
};
...
var ROW_H = 30, HEAD_H = 34, PHASE_H = 30, LABEL_W = 300, COL_W = 150, BAR_H = 18, PAD = 8;
...
export default function SetupGuidePage() {
  ...
  return (
    <div style={{ color: '#12232E' }}>
      <div style={{ background: 'white', border: '1px solid #D2DCE3', borderRadius: '10px', padding: '16px 20px', marginBottom: '14px' }}>
        <div style={{ fontSize: '19px', fontWeight: 700, marginBottom: '3px' }}>Set Up Guide</div>
        <div style={{ fontSize: '12.5px', color: '#5C6F7C', lineHeight: 1.5, marginBottom: '10px' }}>Everything this city must set up before it can answer records requests for real, in the order it has to happen. Steps are order, not dates: a bar in step 3 means two things come before it. Arrows are the dependencies; dashed lines are the last steps feeding go-live. Click any row to open the screen that sets it.</div>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
          {Object.keys(STATE).map(function (k) {
            var s = STATE[k];
            return <span key={k} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '11.5px', color: '#5C6F7C' }}><span style={{ width: '14px', height: '10px', borderRadius: '3px', background: s.bg, border: '1.5px ' + (s.dashed ? 'dashed' : 'solid') + ' ' + s.bd }} />{s.label}{counts[k] != null ? ' · ' + counts[k] : ''}</span>;
          })}
        </div>
      </div>
      ...
      <div style={{ background: 'white', border: '1px solid #D2DCE3', borderRadius: '10px', overflowX: 'auto' }}>
        <div style={{ position: 'relative', width: width + 'px', height: totalH + 'px', minWidth: '100%' }}>
          ...
          {rows.map(function (r, i) {
            ...
            var it = r.item, s = STATE[it.state] || STATE.not_started, st = stepOf[it.key];
            var open = it.door ? function () { nav(it.door); } : null;
            return (
              <div key={it.key} onClick={open} title={it.evidence || ''} style={{ position: 'absolute', left: 0, top: r.y + 'px', width: '100%', height: ROW_H + 'px', cursor: open ? 'pointer' : 'default' }}
                ...
                <div style={{ position: 'absolute', left: '14px', top: 0, width: (LABEL_W - 20) + 'px', height: ROW_H + 'px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: s.dot, flexShrink: 0, border: s.dashed ? '1px dashed #9AA8B4' : 'none' }} />
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', color: it.noScreen ? '#8296A4' : '#12232E' }}>{it.name}{it.noScreen ? ' — no screen yet' : ''}</span>
                </div>
                <div style={{ position: 'absolute', left: barX(st) + 'px', top: ((ROW_H - BAR_H) / 2) + 'px', width: (COL_W - 2 * PAD) + 'px', height: BAR_H + 'px', borderRadius: '5px', background: s.bg, border: '1.5px ' + (s.dashed ? 'dashed' : 'solid') + ' ' + s.bd, boxSizing: 'border-box' }} />
              </div>
            );
          })}
          {/* arrows */}

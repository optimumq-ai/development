...
export default function SetupHubPage() {
  ...
  if (!data) return <div style={{ color: 'var(--oq-fg-9ca3af)', padding: '24px' }}>Loading…</div>;
  var byKey = {};
  ...
  return (
    <div style={{ color: 'var(--oq-fg-12232e)' }}>
      ...
      <div style={{ fontSize: '13px', color: 'var(--oq-fg-5c6f7c)', marginBottom: '18px', lineHeight: 1.5 }}>Every screen that shapes how the system runs, grouped by what it is about. Initial setup and its approvals happen on the Set Up Guide; this page is the way back to any screen afterwards.</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '14px' }}>
        {groups.map(function (gr) {
          return (
            <div key={gr.title} style={{ background: 'var(--oq-bg-ffffff)', border: '1px solid var(--oq-ln-d2dce3)', borderRadius: '10px', overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', fontSize: '13px', fontWeight: 700, color: 'var(--oq-fg-1e6091)', background: 'var(--oq-bg-f2f6f9)' }}>{gr.title}</div>
              {gr.rows.map(function (row, i) {
                var open = row.door ? function () { nav(row.door); } : null;
                return (
                  <div key={row.name + i} onClick={open} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderTop: '1px solid var(--oq-ln-eef2f5)', fontSize: '13px', cursor: open ? 'pointer' : 'default', color: open ? 'var(--oq-fg-12232e)' : 'var(--oq-fg-8296a4)' }}
                    onMouseEnter={function (e) { if (open) e.currentTarget.style.background = 'var(--oq-x-f8fafc)'; }} onMouseLeave={function (e) { e.currentTarget.style.background = 'var(--oq-bg-ffffff)'; }}>
                    <span style={{ flexGrow: 1 }}>{row.name}</span>
                    {row.hint ? <span style={{ fontSize: '12px', color: 'var(--oq-fg-8296a4)' }}>{row.hint}</span> : null}
                    {open ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--oq-fg-8296a4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6" /></svg> : null}
                  </div>
                );
              })}
            </div>

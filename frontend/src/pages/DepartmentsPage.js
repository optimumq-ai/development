    ...
    return (
      <Modal onClose={function(){ if(!saving) setEditor(null); }}>
        ...
        {kind==='team' ? (
          ...
          <Field label="Fulfills requests for (routing) \u2014 this team handles these departments\u2019 requests"><div style={{display:'flex',flexDirection:'column',gap:'5px',maxHeight:'160px',overflowY:'auto',border:'1px solid #E5E7EB',borderRadius:'8px',padding:'8px'}}>{depts().map(function(x){ var on=(d.fulfills_for||[]).indexOf(x.id)>=0; return <label key={x.id} style={{fontSize:'13px',display:'flex',alignItems:'center',gap:'7px',cursor:'pointer'}}><input type="checkbox" checked={on} onChange={function(){ var cur=(d.fulfills_for||[]).slice(); var i=cur.indexOf(x.id); if(i>=0)cur.splice(i,1); else cur.push(x.id); setField('fulfills_for',cur); }}/> {x.name}</label>; })}</div></Field>

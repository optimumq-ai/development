...
export default function RedactionWorkspacePage() {
  ...
  if (loading) return <div style={{ padding: '48px', textAlign: 'center', color: 'var(--oq-fg-9ca3af)' }}>Opening document...</div>;
  ...
  return (
    <div style={{ height: '100%', minHeight: 'calc(100vh - 140px)', display: 'flex', flexDirection: 'column', background: 'var(--oq-bg-f3f4f6)' }}>
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 20px', background: 'var(--oq-bg-ffffff)', borderBottom: '1px solid var(--oq-ln-e5e7eb)', flexShrink: 0 }}>
        <button onClick={function () { job && job.request_id ? nav('/requests/' + job.request_id) : nav(-1); }} style={{ border: '1px solid var(--oq-ln-e5e7eb)', background: 'var(--oq-bg-ffffff)', borderRadius: '8px', padding: '7px 12px', fontSize: '13px', fontWeight: '600', color: 'var(--oq-fg-374151)', cursor: 'pointer' }}>&larr; Back</button>
        <div style={{ fontWeight: '700', fontSize: '15px', color: 'var(--oq-fg-111111)', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Redaction Workspace</div>
        {pages.length > 1 ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button onClick={function () { gotoPage(pageIdx - 1); }} disabled={pageIdx === 0} style={{ border: '1px solid var(--oq-ln-e5e7eb)', background: 'var(--oq-bg-ffffff)', borderRadius: '6px', padding: '5px 10px', cursor: 'pointer' }}>&lsaquo;</button>
            <span style={{ fontSize: '13px', color: 'var(--oq-fg-6b7280)' }}>Page {pageIdx + 1} / {pages.length}</span>
            <button onClick={function () { gotoPage(pageIdx + 1); }} disabled={pageIdx === pages.length - 1} style={{ border: '1px solid var(--oq-ln-e5e7eb)', background: 'var(--oq-bg-ffffff)', borderRadius: '6px', padding: '5px 10px', cursor: 'pointer' }}>&rsaquo;</button>
          </div>
        ) : null}
      </div>

      {error ? <div style={{ background: 'var(--oq-bg-fef2f2)', color: 'var(--oq-fg-991b1b)', padding: '10px 20px', fontSize: '13px' }}>{error}</div> : null}
      {matchTpl ? (
        <div style={{ background: 'var(--oq-bg-eff6ff)', borderBottom: '1px solid var(--oq-ln-bfdbfe)', padding: '10px 20px', fontSize: '13px', color: 'var(--oq-fg-1e40af)', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <button onClick={applyTemplate} disabled={matchBusy} style={{ flexShrink: 0, padding: '6px 12px', borderRadius: '7px', border: 'none', background: matchBusy ? 'var(--oq-bg-9cb4cc)' : 'var(--oq-bg-1f4e79)', color: 'var(--oq-fg-ffffff)', fontSize: '12.5px', fontWeight: '700', cursor: 'pointer' }}>{matchBusy ? 'Applying...' : 'Apply template'}</button>
          <button onClick={function () { setMatchTpl(null); }} style={{ flexShrink: 0, padding: '6px 12px', borderRadius: '7px', border: '1px solid var(--oq-ln-bfdbfe)', background: 'var(--oq-bg-ffffff)', color: 'var(--oq-fg-1e40af)', fontSize: '12.5px', fontWeight: '600', cursor: 'pointer' }}>Not this form</button>
        </div>
      ) : null}
      ...
      {tplOpen ? (
        <div onClick={function () { if (!savingTpl) setTplOpen(false); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div onClick={function (e) { e.stopPropagation(); }} style={{ background: 'var(--oq-bg-ffffff)', borderRadius: '12px', padding: '22px', width: '440px', maxWidth: '92%' }}>
            ...
            <p style={{ fontSize: '12.5px', color: 'var(--oq-fg-6b7280)', margin: '0 0 14px', lineHeight: 1.5 }}>Saves these {zones.length} box(es) and their rules as a template you can reuse on other documents of the same form type.</p>
            <label style={{ fontSize: '12px', fontWeight: '600', color: 'var(--oq-fg-374151)' }}>Template name</label>
            <input value={tplName} onChange={function (e) { setTplName(e.target.value); }} placeholder="e.g. PD Incident Report - PII" style={{ width: '100%', boxSizing: 'border-box', padding: '8px 10px', border: '1px solid var(--oq-ln-e5e7eb)', borderRadius: '8px', fontSize: '13px', margin: '4px 0 12px' }} />
            <label style={{ fontSize: '12px', fontWeight: '600', color: 'var(--oq-fg-374151)' }}>Description (optional)</label>
            <input value={tplDesc} onChange={function (e) { setTplDesc(e.target.value); }} placeholder="What this template redacts" style={{ width: '100%', boxSizing: 'border-box', padding: '8px 10px', border: '1px solid var(--oq-ln-e5e7eb)', borderRadius: '8px', fontSize: '13px', margin: '4px 0 16px' }} />
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '24px', display: 'flex', justifyContent: 'center' }}>
          {page ? (
            <div ref={wrapRef} onMouseDown={onDown} onMouseMove={onMove} onMouseUp={onUp} onMouseLeave={function () { setDraft(null); }}
              style={{ position: 'relative', width: '720px', maxWidth: '100%', alignSelf: 'flex-start', boxShadow: '0 2px 10px rgba(0,0,0,.12)', cursor: 'crosshair', userSelect: 'none', background: 'var(--oq-bg-ffffff)' }}>
              {imgUrls[page.id] ? <img src={imgUrls[page.id]} alt={'Page ' + page.page_no} draggable={false} style={{ width: '100%', display: 'block', pointerEvents: 'none' }} /> : <div style={{ width: '100%', aspectRatio: '8.5/11', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--oq-fg-9ca3af)' }}>Loading page...</div>}
              {pageZones.map(function (z) {
                ...
                return (
                  <div key={z.id} onMouseDown={function (e) { startMove(e, z); }} style={{ position: 'absolute', left: pct(z.x), top: pct(z.y), width: pct(z.w), height: pct(z.h), background: 'rgba(0,0,0,.78)', border: '1px solid ' + hc, boxSizing: 'border-box', pointerEvents: 'auto', cursor: 'move', display: 'flex', alignItems: 'center' }}>
                    <span style={{ marginLeft: '2px', width: '15px', height: '15px', minWidth: '15px', borderRadius: '50%', background: 'var(--oq-bg-ffffff)', color: 'var(--oq-fg-111111)', fontSize: '9px', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1, pointerEvents: 'none' }}>{numById[z.id]}</span>
              ...
              {pageSuggestions.map(function (s) {
                return <div key={s._k} style={{ position: 'absolute', left: pct(s.x), top: pct(s.y), width: pct(s.w), height: pct(s.h), border: '1.5px dashed ' + (s.category && CAT_COLORS[s.category] ? CAT_COLORS[s.category] : 'var(--oq-ln-b45309)'), background: 'rgba(245,158,11,.20)', boxSizing: 'border-box', pointerEvents: 'none', borderRadius: '2px' }} />;
              })}
              {draftBox ? <div style={{ position: 'absolute', left: draftBox.left, top: draftBox.top, width: draftBox.width, height: draftBox.height, background: 'rgba(31,78,121,.25)', border: '1px dashed var(--oq-ln-1f4e79)', pointerEvents: 'none' }} /> : null}
            </div>
          ) : <div style={{ color: 'var(--oq-fg-9ca3af)' }}>No pages.</div>}
        </div>

        {/* Sidebar */}
        <div style={{ width: '320px', flexShrink: 0, background: 'var(--oq-bg-ffffff)', borderLeft: '1px solid var(--oq-ln-e5e7eb)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--oq-ln-f3f4f6)' }}>
            <div style={{ fontSize: '12px', fontWeight: '700', color: 'var(--oq-fg-374151)', marginBottom: '6px' }}>RULE FOR NEW BOXES</div>
            <select value={ruleId} onChange={function (e) { setRuleId(e.target.value); }} style={{ width: '100%', padding: '8px 10px', border: '1px solid var(--oq-ln-e5e7eb)', borderRadius: '8px', fontSize: '13px', background: 'var(--oq-bg-ffffff)' }}>
              <option value="">(No rule / manual)</option>
              {rules.map(function (r) { return <option key={r.id} value={r.id}>{r.title} ({r.category_label})</option>; })}
            </select>
            <p style={{ fontSize: '11.5px', color: 'var(--oq-fg-9ca3af)', margin: '6px 0 0', lineHeight: 1.4 }}>Drag on the page to draw a box; drag it to move, corners to resize. New boxes get the rule selected above.</p>
            <button onClick={discover} disabled={discovering} style={{ width: '100%', marginTop: '8px', padding: '8px', borderRadius: '8px', border: 'none', background: discovering ? 'var(--oq-bg-9cb4cc)' : 'var(--oq-bg-1f4e79)', color: 'var(--oq-fg-ffffff)', fontSize: '13px', fontWeight: '700', cursor: discovering ? 'wait' : 'pointer' }}>{discovering ? 'Scanning document...' : 'Find exempt content (AI)'}</button>
            {rules.length === 0 ? <p style={{ fontSize: '12px', color: 'var(--oq-fg-b45309)', margin: '8px 0 0' }}>No approved + active rules yet. Boxes will be unlabeled until you attach a rule.</p> : null}
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '14px 18px' }}>
            {pageSuggestions.length ? (
              <div style={{ marginBottom: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <span style={{ fontSize: '12px', fontWeight: '700', color: 'var(--oq-fg-b45309)' }}>AI SUGGESTIONS ({pageSuggestions.length})</span>
                  <button onClick={acceptAllOnPage} style={{ border: 'none', background: 'transparent', color: 'var(--oq-fg-1f4e79)', fontSize: '12px', fontWeight: '700', cursor: 'pointer' }}>Accept all</button>
                </div>
                {pageSuggestions.map(function (s) {
                  return (
                    <div key={s._k} style={{ border: '1px dashed var(--oq-ln-fcd34d)', background: 'var(--oq-bg-fffbeb)', borderRadius: '8px', padding: '8px 10px', marginBottom: '8px' }}>
                      <div style={{ fontSize: '12px', color: 'var(--oq-fg-111111)', fontWeight: '600', marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.text}</div>
                      <div style={{ fontSize: '11px', color: 'var(--oq-fg-6b7280)', marginBottom: '6px' }}>{s.rule_title ? s.rule_title : 'No matching rule - pick one after accepting'}</div>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button onClick={function () { acceptSuggestion(s); }} style={{ flex: 1, padding: '5px', borderRadius: '6px', border: 'none', background: 'var(--oq-bg-1f4e79)', color: 'var(--oq-fg-ffffff)', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>Accept</button>
                        <button onClick={function () { dismissSuggestion(s); }} style={{ flex: 1, padding: '5px', borderRadius: '6px', border: '1px solid var(--oq-ln-e5e7eb)', background: 'var(--oq-bg-ffffff)', color: 'var(--oq-fg-6b7280)', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>Dismiss</button>
                      </div>
                    </div>
                  );
                })}
            ...
            <div style={{ fontSize: '12px', fontWeight: '700', color: 'var(--oq-fg-374151)', marginBottom: '10px' }}>BOXES ON PAGE {pageIdx + 1} ({pageZones.length})</div>
            {pageZones.length === 0 ? <div style={{ fontSize: '13px', color: 'var(--oq-fg-9ca3af)' }}>None yet.</div> : pageZones.map(function (z, i) {
              return (
                <div key={z.id} style={{ border: '1px solid var(--oq-ln-e5e7eb)', borderRadius: '8px', padding: '8px 10px', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                    <span style={{ width: '10px', height: '10px', borderRadius: '2px', background: catColor(z.rule_id), flexShrink: 0 }} />
                    <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '17px', height: '17px', minWidth: '17px', borderRadius: '50%', background: 'var(--oq-bg-111111)', color: 'var(--oq-fg-ffffff)', fontSize: '10px', fontWeight: '700', flexShrink: 0 }}>{numById[z.id]}</span>
                    <span style={{ fontSize: '12px', color: 'var(--oq-fg-6b7280)', flex: 1 }}>{ruleOf(z.rule_id) ? ruleOf(z.rule_id).category_label : 'No rule attached'}</span>
                    <button onClick={function () { delZone(z.id); }} style={{ border: 'none', background: 'transparent', color: 'var(--oq-fg-dc2626)', cursor: 'pointer', fontSize: '13px' }}>Remove</button>
                  </div>
                  <select value={z.rule_id || ''} onChange={function (e) { setZoneRule(z.id, e.target.value); }} style={{ width: '100%', padding: '5px 8px', border: '1px solid var(--oq-ln-e5e7eb)', borderRadius: '6px', fontSize: '12px', background: 'var(--oq-bg-ffffff)' }}>
                    <option value="">(No rule)</option>
                    {rules.map(function (r) { return <option key={r.id} value={r.id}>{r.title}</option>; })}
                  </select>
                  <div style={{ display: 'flex', gap: '6px', marginTop: '6px' }}>
                    <input value={labelDraft[z.id] || ''} onChange={function (e) { var v = e.target.value; setLabelDraft(function (m) { var n = Object.assign({}, m); n[z.id] = v; return n; }); }} onKeyDown={function (e) { if (e.key === 'Enter') pickRuleByLabel(z.id); }} placeholder="Describe field, AI picks rule" style={{ flex: 1, minWidth: 0, padding: '5px 8px', border: '1px solid var(--oq-ln-e5e7eb)', borderRadius: '6px', fontSize: '11.5px' }} />
                    <button onClick={function () { pickRuleByLabel(z.id); }} disabled={labelBusy[z.id] || !(labelDraft[z.id] || '').trim()} style={{ padding: '5px 9px', borderRadius: '6px', border: 'none', background: (labelBusy[z.id] || !(labelDraft[z.id] || '').trim()) ? 'var(--oq-bg-9cb4cc)' : 'var(--oq-bg-1f4e79)', color: 'var(--oq-fg-ffffff)', fontSize: '11.5px', fontWeight: '700', cursor: 'pointer', whiteSpace: 'nowrap' }}>{labelBusy[z.id] ? '...' : 'AI rule'}</button>
                  </div>
                </div>
              );
            })}
          </div>
          <div style={{ padding: '16px 18px', borderTop: '1px solid var(--oq-ln-f3f4f6)' }}>
            {result ? (
              <div>
                <div style={{ background: 'var(--oq-bg-ecfdf5)', border: '1px solid var(--oq-ln-a7f3d0)', borderRadius: '8px', padding: '10px 12px', fontSize: '12.5px', color: 'var(--oq-fg-065f46)', marginBottom: '10px', lineHeight: 1.5 }}>
                  {templateMode
                ...
                <button onClick={download} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: 'none', background: 'var(--oq-bg-1f4e79)', color: 'var(--oq-fg-ffffff)', fontSize: '13px', fontWeight: '600', cursor: 'pointer', marginBottom: '8px' }}>{templateMode ? 'Download Test Output' : 'Download Redacted PDF'}</button>
                {templateMode
                  ? <div>
                      <button onClick={function () { setTplMsg(null); setTplOpen(true); }} style={{ width: '100%', padding: '9px', borderRadius: '8px', border: '1px solid var(--oq-ln-1f4e79)', background: 'var(--oq-bg-ffffff)', color: 'var(--oq-fg-1f4e79)', fontSize: '13px', fontWeight: '600', cursor: 'pointer', marginBottom: '8px' }}>Save as Reusable Template</button>
                      <button onClick={function () { setResult(null); }} style={{ width: '100%', padding: '9px', borderRadius: '8px', border: '1px solid var(--oq-ln-e5e7eb)', background: 'var(--oq-bg-ffffff)', color: 'var(--oq-fg-374151)', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>Keep editing</button>
                    </div>
                  : <button onClick={function () { job && job.request_id ? nav('/requests/' + job.request_id) : nav(-1); }} style={{ width: '100%', padding: '9px', borderRadius: '8px', border: '1px solid var(--oq-ln-e5e7eb)', background: 'var(--oq-bg-ffffff)', color: 'var(--oq-fg-374151)', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>Done</button>}
              </div>
            ) : (
              <div>
                <button onClick={apply} disabled={applying || zones.length === 0} style={{ width: '100%', padding: '11px', borderRadius: '8px', border: 'none', background: (applying || zones.length === 0) ? 'var(--oq-bg-9cb4cc)' : 'var(--oq-bg-1f4e79)', color: 'var(--oq-fg-ffffff)', fontSize: '14px', fontWeight: '700', cursor: (applying || zones.length === 0) ? 'default' : 'pointer' }}>{applying ? (templateMode ? 'Generating…' : 'Applying...') : (templateMode ? 'Test Redaction Output (' + zones.length + ')' : 'Apply Redaction (' + zones.length + ')')}</button>
                <button onClick={function () { setTplMsg(null); setTplOpen(true); }} disabled={zones.length === 0} style={{ width: '100%', marginTop: '8px', padding: '9px', borderRadius: '8px', border: '1px solid var(--oq-ln-1f4e79)', background: 'var(--oq-bg-ffffff)', color: 'var(--oq-fg-1f4e79)', fontSize: '13px', fontWeight: '600', cursor: zones.length === 0 ? 'default' : 'pointer', opacity: zones.length === 0 ? 0.5 : 1 }}>Save as Reusable Template</button>
              </div>
            )}
          </div>
        </div>

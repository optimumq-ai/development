...
var ta = { width: '100%', minHeight: '70px', padding: '10px', border: '1px solid #D1D5DB', borderRadius: '8px', fontSize: '13px', fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box' };
var small = function (color, border) { return { padding: '5px 10px', fontSize: '12px', background: 'white', color: color, border: '1px solid ' + border, borderRadius: '6px', cursor: 'pointer', fontFamily: 'inherit' }; };
...
export default function AgentRulesPage() {
  ...
  return (
    <SetupScreen hubKey="agent_rules" laneLabel="System Features and Options" title="Portal Agent Rules"
      ...
      {function (s) {
        if (!rules) return <div style={{ color: '#9CA3AF' }}>Loading…</div>;
        return (
          <div>
            {msg ? <Msg text={msg.text} ok={msg.ok} /> : null}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px' }}>
              {rules.length === 0 ? <div style={{ fontSize: '13px', color: '#9CA3AF', fontStyle: 'italic', padding: '8px 0' }}>No rules yet. Add one below.</div> : null}
              {rules.map(function (r) {
                var isEditing = editing === r.id;
                return (
                  <div key={r.id} style={{ border: '1px solid #D2DCE3', borderRadius: '8px', padding: '12px', background: r.enabled ? 'white' : '#F9FAFB', opacity: r.enabled ? 1 : 0.65 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '11px', color: '#8296A4', marginTop: '6px' }}>{r.created_by ? 'Added by ' + r.created_by : ''} {r.created_at ? '· ' + new Date(r.created_at + 'Z').toLocaleDateString() : ''}</div>
                      </div>
                      {s.can ? (
                        <div style={{ display: 'flex', gap: '6px' }}>
                          {isEditing
                            ? <><button type="button" onClick={function () { saveEdit(r.id, s.reload); }} style={Object.assign({}, small('white', BLUE), { background: BLUE })}>Save</button><button type="button" onClick={function () { setEditing(null); setEditText(''); }} style={small('#6B7280', '#D1D5DB')}>Cancel</button></>
                            : <><button type="button" onClick={function () { setEditing(r.id); setEditText(r.rule_text); }} style={small(BLUE, BLUE)}>Edit</button><button type="button" onClick={function () { remove(r.id, s.reload); }} style={small('#B91C1C', '#FCA5A5')}>Delete</button></>}
                        </div>
                      ) : null}
                    </div>
                  </div>
            ...
            <div style={{ borderTop: '1px solid #EEF2F5', paddingTop: '16px' }}>

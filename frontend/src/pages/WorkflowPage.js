...
function btn(disabled){ return {padding:'9px 18px',borderRadius:'8px',border:'none',background:disabled?'var(--oq-bg-9ca3af)':NAVY,color:'var(--oq-fg-ffffff)',fontSize:'13px',fontWeight:'600',cursor:disabled?'default':'pointer'}; }
var btnGhost = {padding:'9px 16px',borderRadius:'8px',border:'1px solid var(--oq-ln-e5e7eb)',background:'var(--oq-bg-ffffff)',color:'var(--oq-fg-6b7280)',fontSize:'13px',fontWeight:'600',cursor:'pointer'};
...
export default function WorkflowPage(){
  ...
  return (
    ...
    <div>
      ...
      <div style={{background:'var(--oq-bg-ffffff)',border:'1px solid var(--oq-ln-dbeafe)',borderRadius:'12px',padding:'18px 20px',marginBottom:'20px'}}>
        ...
        {err&&<div style={{fontSize:'13px',color:'var(--oq-fg-9b1c1c)',marginTop:'10px'}}>{err}</div>}
        {draft&&(
          <div style={{marginTop:'14px',border:'1px solid var(--oq-ln-e5e7eb)',borderRadius:'10px',padding:'14px',background:'var(--oq-bg-f8faff)'}}>
            <div style={{fontSize:'13px',fontWeight:'700',color:'var(--oq-fg-111111)'}}>{draft.name}</div>
            <div style={{fontSize:'13px',color:'var(--oq-fg-374151)',margin:'4px 0 10px',lineHeight:'1.5'}}>{draft.description}</div>
            <div style={{fontSize:'12px',color:'var(--oq-fg-6b7280)'}}><b>When:</b> {(draft.conditions||[]).length? (draft.conditions||[]).map(condText).join('  AND  ') : 'always (catch-all)'}</div>
            <div style={{fontSize:'12px',color:'var(--oq-fg-6b7280)',marginTop:'4px'}}><b>Then:</b> route to {STAGE_LABEL[draft.actions&&draft.actions.stage]||(draft.actions&&draft.actions.stage)||'-'} at {teamLabel(draft.actions&&draft.actions.team)}</div>
            {(draft.warnings&&draft.warnings.length>0)&&<div style={{fontSize:'12px',color:'var(--oq-fg-92400e)',background:'var(--oq-bg-fef3c7)',borderRadius:'8px',padding:'8px 10px',marginTop:'10px'}}>{draft.warnings.map(function(w,i){return <div key={i}>&ndash; {w}</div>;})}</div>}
      ...
      {loading? <div style={{color:'var(--oq-fg-9ca3af)'}}>Loading...</div> : (
        <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
          {rules.map(function(r){
            return (
              <div key={r.id} style={{background:'var(--oq-bg-ffffff)',border:'1px solid var(--oq-ln-e5e7eb)',borderRadius:'10px',padding:'14px 16px',opacity:r.enabled?1:0.6}}>
                <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                  <input type="number" defaultValue={r.priority} onBlur={function(e){ if(Number(e.target.value)!==r.priority) setPriority(r,e.target.value); }} title="Priority (lower runs first)"
                    style={{width:'56px',padding:'4px 6px',border:'1px solid var(--oq-ln-e5e7eb)',borderRadius:'6px',fontSize:'13px',textAlign:'center'}}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:'14px',fontWeight:'700',color:'var(--oq-fg-111111)'}}>{r.name}{r.source==='seed'&&<span style={chip('var(--oq-fg-eff6ff)',NAVY)}>built-in</span>}{r.source==='ai-authored'&&<span style={chip('var(--oq-fg-f3e8ff)','var(--oq-fg-6d28d9)')}>AI-authored</span>}</div>
                    {r.description&&<div style={{fontSize:'12px',color:'var(--oq-fg-6b7280)',marginTop:'2px',lineHeight:'1.5'}}>{r.description}</div>}
                  </div>
                  <label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'12px',color:'var(--oq-fg-6b7280)',cursor:'pointer'}}>
                    <input type="checkbox" checked={!!r.enabled} onChange={function(){toggle(r);}}/> {r.enabled?'On':'Off'}
                  </label>
                  <button onClick={function(){del(r);}} style={{padding:'4px 10px',background:'var(--oq-bg-ffffff)',color:'var(--oq-fg-9b1c1c)',border:'1px solid var(--oq-ln-fca5a5)',borderRadius:'6px',fontSize:'12px',fontWeight:'600',cursor:'pointer'}}>Delete</button>
                </div>
                <div style={{fontSize:'12px',color:'var(--oq-fg-6b7280)',marginTop:'10px',paddingTop:'10px',borderTop:'1px solid var(--oq-ln-f3f4f6)',lineHeight:'1.7'}}>

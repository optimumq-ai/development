...
export default function AppLayout() {
  ...
  const asideStyle = {
    ...
    display: 'flex', flexDirection: 'column', background: 'white', borderRight: '1px solid #E5E7EB',
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#F9FAFB', overflow: 'hidden', position: 'relative' }}>
      ...
      <aside style={asideStyle} onMouseEnter={enter} onMouseLeave={leave} onFocus={enter} onBlur={leave} aria-label="Main navigation">
        ...
        <nav style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '10px 8px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {items.map(function (item) {
            if (item.header) {
              return open
                ? <div key={item.header} style={{ margin: '10px 2px 2px', padding: '0 8px', fontSize: '10.5px', fontWeight: '800', letterSpacing: '.07em', textTransform: 'uppercase', color: '#9CA3AF', whiteSpace: 'nowrap' }}>{item.header}</div>
                : <div key={item.header} title={item.header} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '36px', margin: '6px 0 0', borderRadius: '8px', color: groupActive(item) ? '#1F4E79' : '#9CA3AF', background: groupActive(item) ? '#EEF3F8' : 'none' }}>
            ...
            return (
              <NavLink key={item.to} to={item.to} title={item.label} end={item.to === '/requests'}
                style={function (a) { return { display: 'flex', alignItems: 'center', gap: '10px', justifyContent: open ? 'flex-start' : 'center', padding: open ? '8px 10px' : '8px 0', paddingLeft: open && item.child ? '22px' : (open ? '10px' : '0'), borderRadius: '8px', textDecoration: 'none', fontSize: '13px', fontWeight: a.isActive ? '600' : '500', color: navColor(a.isActive), background: a.isActive ? '#EEF3F8' : 'none', whiteSpace: 'nowrap', minHeight: '36px', boxSizing: 'border-box' }; }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        <header style={{ background: 'white', borderBottom: '1px solid #E5E7EB', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, height: '57px', boxSizing: 'border-box' }}>
          <div style={{ fontSize: '13px', fontWeight: '600', color: '#9CA3AF' }}>{agencyName} · Public Records Management</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <NavLink to="/my-tasks" style={({ isActive }) => ({ display: 'flex', alignItems: 'center', gap: '7px', padding: '6px 12px', borderRadius: '8px', textDecoration: 'none', fontSize: '13px', fontWeight: '600', color: isActive ? '#1F4E79' : '#374151', background: isActive ? '#EEF3F8' : '#F3F4F6', border: '1px solid ' + (isActive ? '#C5D3E1' : '#E5E7EB') })}>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={function () { setUMenu(!uMenu); }} aria-label="Account menu"
                style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 8px 4px 4px', borderRadius: '8px', background: uMenu ? '#F3F4F6' : 'none', border: '1px solid ' + (uMenu ? '#E5E7EB' : 'transparent'), cursor: 'pointer' }}>
                ...
                <div style={{ textAlign: 'left', minWidth: 0 }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color: '#111', whiteSpace: 'nowrap' }}>{user?.display_name}</div>
                  <div style={{ fontSize: '10.5px', color: '#9CA3AF', whiteSpace: 'nowrap' }}>{user?.userTypes?.[0]?.displayName || (user?.userTypes?.length === 0 ? 'No user type' : '')}</div>
                </div>
              </button>
              {uMenu ? (
                <div style={{ position: 'absolute', right: 0, top: '44px', minWidth: '180px', background: 'white', border: '1px solid #E5E7EB', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,.1)', zIndex: 50 }}>

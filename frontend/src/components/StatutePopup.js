  ...
  return (
    <div onClick={props.onClose} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(18,35,46,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div onClick={function (e) { e.stopPropagation(); }} style={{ background: 'var(--oq-bg-ffffff)', borderRadius: '12px', padding: '20px 22px', width: '680px', maxWidth: '94%', maxHeight: '84vh', overflowY: 'auto', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
        ...
        {/* An item the law is silent on has no findings to list, but the research still says what was searched and
            what was (not) found — that note used to vanish with the empty list (Kevin 2026-09-13: Minimum fee popup blank). */}
        {!(info.text && info.text.length) && info.notes ? <div style={{ borderTop: '1px solid ' + C.line, marginTop: '12px', paddingTop: '10px', fontSize: '12.5px', color: C.ink }}>
          <div style={{ fontSize: '10.5px', fontWeight: '800', letterSpacing: '.05em', textTransform: 'uppercase', color: C.mute, marginBottom: '4px' }}>What the research found</div>
          <div>{info.notes}</div>
        </div> : null}
        {!recs ? <div style={{ fontSize: '12.5px', color: C.ph, marginTop: '12px' }}>Reading the research record…</div> : null}
        {recs && !recs.some(function (x) { return x.rule; }) && !(info.text && info.text.length) && !info.notes ? <div style={{ fontSize: '12.5px', color: C.mute, marginTop: '12px' }}>No research record is attached to this item beyond its citation.</div> : null}

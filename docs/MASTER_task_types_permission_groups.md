...
| `legal_redaction` | Legal Redaction (advanced) | *(new — task-type key)* | ORO Legal Associate; ORO Senior Legal | trigger: `legal_flag` (Director escalation), `SENSITIVE`/`LEGAL_HOLD` in latest `workflow_decisions.flags`, or `record_types.legal_redaction_required` (the deterministic per-type toggle, item 10 `[BUILT 2026-08-13]`); replaces `redaction` at the redaction stage | routing `[BUILT 2026-07-09]` (team-agnostic, pools to legal staff by subset); dedicated screen `[NOT BUILT]` |
...
| `intake_review` | Intake Review | *(task-type key)* | **ORO Associate** | trigger-spawned, team-agnostic. Triggers (enum `unroutable` \| `eligibility_review` \| `approval_pending` \| `sensitivity_flag` \| `reopen_retriage` \| `legal_rt`) recorded on the task; `intake_review_mode` = `when_needed` (default) \| `always`. `legal_rt` `[ADDED 2026-08-14]` is the deterministic twin of `sensitivity_flag`: the pinned record type carries `legal_redaction_required` (own or inherited from its parent bucket — same walk-up as the redaction-stage escalation, `taskRouting.recordTypeLegalGate`), so legal hours are expectable and belong in front of intake eyes at pricing time (DESIGN_legal_hours_estimate.md slice 3) | routing `[BUILT 2026-07-29 BW2]`; screen `[BUILT 2026-07-30 BW3 — IntakeReviewTaskPage; stale NOT-BUILT note corrected 2026-08-14]` |

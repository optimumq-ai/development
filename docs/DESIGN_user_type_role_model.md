...
**Critical distinction — Open Records *Office* vs Open Records *Fulfillment Team*:** `[REALIZED IN DATA 2026-08-15]`
- The **Open Records Office** = management/administration of all fulfillment. It **does not process requests**, so it must **never appear in the Taxonomy** — nothing routes to it. (It DOES manage MRRs — Kevin 2026-08-15: an ORO Associate manages the MRR and generates the roll-up estimate; fulfillment teams do each child's search/redaction/labor estimate. Compatible because MRR child work is hand-assigned, never team-routed.)
- But for **staff assignment**, the Office must be selectable as a "team" (people belong to it) even though it is **not a fulfillment team**. `[BUILT 2026-08-15]`: `dept-openrecords` is now the **Open Records Office** (staffing-only: serves no departments, `is_open_records=0`; holds the director/administrators), and a new **`team-openrecords` "Open Records Fulfillment Team"** carries `is_open_records=1` (the routing fallback) and serves the small/low-volume departments (Building & Planning, Public Works, PIO, Parks & Rec). Michael Hargrove + Tom Jones staff it (Kevin's allocation). Org page Teams tab (renamed from "Fulfillment Teams") pins the Office on top over a "Fulfillment Teams" section, and every team row has **View staff** — a read-only member popup ("Go to STAFF to modify team member list."). Seeds updated (`seed_teams_provisional.sql`).

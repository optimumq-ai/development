...
`/home/optimumq/exchange`) — two screens: the section readiness index, and the confirm-each-policy-setting
section detail (clarification as the worked example). Single city; Columbus renders identically
with its own 9 warnings.

This is rule (d)'s surface from the Phase-7 handoff: every ⚠ local policy setting imports as
`confirmed: false` with a `suggested_default`; a section cannot reach `configured` — and `attest()`
refuses it — while any policy setting is unconfirmed. TX and OH each carry 9 such warnings today. The
...
- Kevin's question ("do we have mockups for each section corresponding to the rules-engine
  ...
  (content · policy settings · provenance · proposals). **This draft's screen 2 (the policy-setting cards) becomes
  the City-policy settings zone of Draft 10's section screen** — one surface per section, not two screens;
  the confirm flow and the attest gate are unchanged.
- New policy setting decided 7/29 (Draft 8 rev 2 §3.5): the **pre-send review gate** ships as an
  unconfirmed ⚠ policy setting, `suggested_default: off` — a named person must consciously confirm the
  automation decision before its section attests.

## 1. The shape

- **Screen 1 — readiness index.** A computed gate summary (unconfirmed policy settings · pending proposals ·
  integrity findings · sections attested · enforcement dev-mode), then one row per
  `jurisdiction_rules` domain (15): status chip, policy-setting count, content owner, attest action.
...
- **Screen 2 — the policy-setting card.** Per policy setting: the note (what the statute left to the city), the
  ...
  with the named reason until every policy setting is confirmed (the `attest()` gate, restated in words).
- **Rail:** pending proposals (one-click into the existing config-proposal review flow) · integrity
  findings (invariants, not policy settings) · enforcement dev-mode state (SADMIN-only act, framed as the
...
| Surface | Binds to |
|---|---|
| Gate summary + section rows | `GET /api/jurisdiction-profile/status` (section readiness index: configured / attested / drifted) + per-domain unconfirmed-policy-setting counts (`jurisdictionProfile` pending-policy-setting walk) |
| Policy setting cards | `jurisdiction_rules` city_config edges: `{note, confirmed, value, suggested_key, suggested_default}`; eligibility dimensions' `confirmed` flag included |
| Confirm action | NEW small endpoint: set policy setting `value` + `confirmed: true` + who/when (today confirmation has no route — the gate exists, the setter doesn't) |
...
| Integrity findings | `GET /api/config-integrity` (invariants 1–5 + unconfirmed-policy-setting findings + active-branch-unconfirmed) |
...
1. **The policy-setting-confirm endpoint** — the one genuinely missing piece of plumbing: `attest()` reads
   `confirmed`, the importer writes it false, and nothing today sets it true.
2. **Per-domain policy setting enumeration** in `/status` (or a sibling) so the index can show counts without

...
### 2.2 Notification
An **ad-hoc item that appears on My Tasks but is NOT a request-processing stop.** It informs and points — a description plus a hyperlink to a relevant screen — rather than presenting a purpose-built completion UI. Used for heads-ups and passive "monitor/observe" items (e.g., "an imported file has arrived — set up a redaction profile"). **Must not depend on a request_id.** `[code: BUILT 2026-07-15 — `notifications` table + `services/notifications.js` (emit/list/read/dismiss, per-user, optional context for dedupe) + `routes/notifications.js` + a header bell. The import "no template yet" prompt is now exactly this notification, replacing the pseudo-request task. `tasks.request_id`/`request_files.request_id` are nullable. `verify_notifications` 18/18. 2026-09-08: the SYSTEM may withdraw a heads-up it sent, for every recipient, once the
thing it asked for has happened or the ask is void (`resolveContext` by kind + context; first use: setup approval,
SPEC_setup_hub §3g). Every list surface (bell, My Tasks) carries a visible per-notification **Dismiss** button for
the rest, and the bell refreshes after any successful write instead of waiting for its 60-second poll.]`

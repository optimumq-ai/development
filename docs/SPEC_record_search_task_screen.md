...
**Address gap — enforced inline, `[STILL NOT BUILT at intake]`:** no mailing address is captured at intake (Name/Email/Phone only; `requests` has no address column). Since email is always required *and* verified, this screen **defaults clarification to email**; the **mail** channel requires a mailing address supplied **inline at send time** — `send()` rejects a postal letter with no address (`400 ADDRESS_REQUIRED`) so the UI can prompt. The inline address is recorded in the effort-trail note but **not persisted** (no column). The broader "capture address when postal delivery is chosen" fix belongs at intake (portal-side) and is flagged separately.
...
| Postal address capture (clarification + delivery) | portal intake + `requests` column | `[NOT BUILT]` (flagged, likely separate slice) |

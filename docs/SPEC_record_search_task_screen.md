...
**Address gap `[NOT BUILT]`:** no mailing address is captured anywhere (intake collects Name/Email/Phone only; `requests` has no address column). Since email is always required *and* verified, this screen **defaults clarification to email**; a postal letter additionally requires capturing/confirming a mailing address inline at send time. The broader "capture address when postal delivery is chosen" fix belongs at intake (portal-side) and is flagged separately.

...
2. [DONE] Redaction data model + Redaction Rules Library (rules + 8 categories + legal_sources many-to-many + pending->approved workflow; API at /api/redaction; aligned to prototype spec (approval_status + is_active separate, 8 category codes, source_document/effective/expiration fields, semicolon-split legal basis stored normalized, activate/deactivate/delete). AI auto-population (auto_redaction_rules_update / redaction_update_reminder_days) still TODO - config keys seeded, schema ready via legal_sources.statute_text + source field). Original line: - redaction_jobs, redaction_zones (page, normalized x/y/w/h, exemption_id, type manual|auto), exemption_reference_library (legal bases), layout_profiles (zone templates, positions only).

...
- TODO (remaining mass-redaction): Document Processing Manager email alert on held/mismatch; Deep Scan (per-doc AI scan of narrative/free-text -> Pending Review bucket); ~~CANDIDATE detection during schema discovery -> suggest as a mass-redaction "opportunity", NOT My Tasks~~ [BUILT 2026-08-13 via the variant-discovery hand-off — flagged variants surface as "Waiting for a template" cards on /mass-redaction; SPEC_taxonomy_classification §1]; batch via fresh UPLOAD (current batch runs over files already in the system); record-type filtering of candidates.
...
**`[DESIGNED 2026-07-13 with Kevin — ready to build]`** — full design in `DESIGN_split_canvas_intake.md`
("Search-completeness intent + refine-and-search-again"). The four decisions:
1. **Refine-and-search-again** — the results canvas carries the query in an **editable field** + **Search again**, plus a "Tried already:" line. Re-search **replaces results**; the **Selected panel accumulates** (dedup by record id). Attach-and-clear now clears on **Proceed**, not on each search: one description can be built from several searches.
2. **Intent captured at Proceed, conditional.** ≥1 selected → the results fade dark and a popup offers two buttons (*"These are all the records I want"* → `complete` · *"Also have the Open Records team search for more"* → `search_more`) plus a *‹ Keep refining* escape. **0 selected → no popup**: the Proceed button itself reads **"Submit to Open Records team for search"** (`no_match_search`). **PATH (b)** (email/AV/photos/data/paper — never searched) is recorded automatically as `not_searchable`, never asked.

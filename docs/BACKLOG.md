...
5. Auto-suggest (assist, not default) - pattern detection (SSN/DOB/phone) + AI. PRESENT APPROACH BEFORE BUILDING (Replit AI zone-discovery was unreliable -> manual stays the default; auto-suggest is assist-only). [DONE - text layer] AI content detection: POST /api/redaction-jobs/file/:fileId/discover -> zoneDiscovery sends page text + approved/active rules menu to claude-sonnet-4-5, gets exempt {text,rule_id} spans, locates each in the extracted word boxes (exact contiguous normalized match) -> SUGGESTED boxes placed on the real text (coords correct by construction, no guessing; the deleted-Replit-build behavior). Workspace: "Find exempt content (AI)" button, dashed amber suggestion boxes, sidebar Accept/Dismiss/Accept-all -> accepted becomes a real numbered zone. Verified on 1095-A: names/addresses/SSNs found, 0 unmatched, SSN auto-mapped to SSN rule (names/addresses came back ruleless because those rules are still pending-not-active). TODO: box ADJUSTABILITY (drag-move + resize handles; currently draw+delete only) + LABEL->AI-RULE (draw a box, name the field e.g. "driver's license number", AI picks the rule from the library) for fields blank on the sample but present on others; SCANNED docs need tesseract OCR (apt; Kevin OK) to get word boxes first.
...
- [DONE] API /api/redaction-templates: create (from workspace zones; builds a simple text fingerprint from the sample doc), list, get, patch, soft-delete, and POST /:id/apply {file_id} = the CONSUME path (creates a job, copies template zones, runs redactionApply -> numbered boxes + Vaughn Index). Verified define->apply end to end.
...
- TODO: dedicated batch wizard (Upload Batch -> Discover -> Review -> Safety Check -> Process -> Results per Replit screenshots); AI zone DISCOVERY as a checklist assist (suggest field types/rules, NOT placement - Replit auto-placement drifted, manual draw/adjust stays default); layout-fingerprint DRIFT CHECK + Document Processing Manager alert + safety threshold (suspend batch on mismatch); Deep Scan (per-doc AI scan of narrative/free-text -> Pending Review bucket); batch apply UI + Public Ready routing; request-fulfillment AUTO-APPLY (record not public-ready + template exists for its record type -> apply, human review, release); CANDIDATE detection during schema discovery (fixed layout + fixed-position sensitive fields + NO free text -> suggest as mass-redaction candidate; surface as a non-task "opportunity", NOT My Tasks).

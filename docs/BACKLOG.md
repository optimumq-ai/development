...
5. Auto-suggest (assist, not default) - pattern detection (SSN/DOB/phone) + AI. PRESENT APPROACH BEFORE BUILDING (Replit AI zone-discovery was unreliable -> manual stays the default; auto-suggest is assist-only). Build the text-layer version first (uses existing document_pages word boxes + regex + AI, NO install); defer the scanned-doc path until Kevin OKs the tesseract apt install (archive.ubuntu.com allowed).

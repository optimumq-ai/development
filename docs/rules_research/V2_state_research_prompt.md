...
| 7a | **Constraint Basis** ⭐ (parameters only) | How the state binds the value — `fixed` (a set amount), `ceiling` ("shall not exceed X"), `floor` (a minimum), or `soft-standard` ("reasonable" — no number). **The state gives a constraint; the CITY supplies the operative value under it** (ruled 2026-07-21). This is why a ceiling-25¢ rule and a fixed-25¢ rule are the **same concept** with different basis — never split them. Put the bound in Clock Spec's sibling sense (e.g. `ceiling: $0.25/page`); leave `soft-standard` without a number. |

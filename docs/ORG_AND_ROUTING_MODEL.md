...
- **#3 reassignment re-route: LIVE-VERIFIED** (2026-07-01). Reassigned a request from Open Records to Archives; the active task followed to Archives and Smart Routing auto-assigned it to Thomas Jackson (paper specialist). Full chain confirmed: reassign -> task follows -> Smart Routing -> person.

## 13. Redaction: auto-population vs. required review/approval (clarified 2026-07-01)

Two distinct layers - do NOT conflate them:

- **Redaction Rules Library (jurisdiction-based, auto-populated):** `ruleDiscovery.js` asks the model for jurisdiction-appropriate open-records exemptions not already present and inserts each as a DRAFT (`source='ai'`, `pending_review`, inactive) tied to `jurisdiction_id`. **Pre-population is automated to the system's best effort; human review, addition/deletion, and approval are REQUIRED - rules are never auto-approved and do not take effect until a supervisor verifies and approves them.** (~26 rules across 8 categories seeded for Texas.) This is exactly the approval gate in the wizard's Redaction Readiness phase.

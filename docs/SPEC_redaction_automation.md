...
3. **Eager computation at stage entry + record-type-clean (c)** `[NEW]` — invoke disposition per responsive file from the redaction orchestrator: run the identity bypass (slice 2), else ensure a job + run the AI read and compute the disposition (this is where case **(c)** `auto_release_eligible`+clean-read lands); suppress task spawn and `advanceIfAllReleased` when all bypass.

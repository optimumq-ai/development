...
- **Template match in triage (slice 3b)** — NOT wired: `redactionTriage.assembleSignals` omits `templateMatched`, so a span-bearing doc with no confident template lands in **Elevated** (safe: more review, never less). Wiring `redactionTemplates.engine.safetyScore` in would let template-covered docs settle to Standard/Simple. Follow-up refinement.

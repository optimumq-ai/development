...
**Follow-ups:** the `redaction_qa` reviewer task still opens the generic page (a reviewer-mode of this screen with approve/return is a follow-up); the AI proposal list has **no page anchor** — document-wide, no page number, no click-to-jump, which does not scale to a 50-page document; the AI-scan spinner references an undefined `spin` keyframe (static, cosmetic); pending Kevin's further markup for visual refinements. Design mockup: `docs/mockups/redaction_screen.html` (from Kevin's discussion PDF) — full-bleed workstation (no left nav), auto-run AI on open, 3-box accordion (AI Redaction with checkbox-select + apply-selected · Manual Redaction with rule-for-new-boxes · Finalize & Release), informational side-by-side (Original vs Proposed, read-only), renamed "Search inside document" modal, and a disposition badge whose Finalize primary action adapts (Simple/Standard → Approve & Release · Elevated → Submit for Review · Legal → Send for Legal Review). Pending Kevin's markup before build.

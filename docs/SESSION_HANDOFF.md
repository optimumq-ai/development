...
## Config Freshness Slices B + C (BUILT 2026-06-24)
B: configExtractors.js per-domain adapter framework (fetch paste/URL + sha256 drift + snapshots; fee/deadline/exemption apply, redaction/taxonomy review-only); endpoints check/extract/proposal-detail/attested-apply. C: RuleUpdatesPage at /rule-updates - sources + check-now + paste-a-document + pending list + 2-step review modal (editable proposed-vs-current -> disclaimer + agree -> apply). Fee apply + deadline extract verified live. The review/apply flow IS the lightweight attestation gate. NEXT (Slice D / future): native-editor apply for redaction+taxonomy, scheduled auto-extract toggle (freshness_auto_extract), file-upload ingestion, and folding sources+attestation into the Jurisdiction Profile model.
...
Approved changes can be scheduled for a future effective date and deploy automatically (hourly promotion + startup). Tables scheduled_config_changes + config_history (effective windows for "what was in effect on date X"). effectiveConfig service (schedule/promoteDue/cancel/applyConfig/recordHistory/seedBaselineHistory/startPromotionScheduler). Approve endpoint branches schedule-vs-now; GET /scheduled, POST /scheduled/:id/cancel, POST /promote. UI: Apply-now/Schedule choice in approve step (live areas) + Scheduled changes list with Cancel. Baseline history seeded for jur-tx. NEXT optional: history viewer + per-section scheduled indicator on the profile dashboard.

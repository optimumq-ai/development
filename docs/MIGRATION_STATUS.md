...
- Converted to async/await (still run on SQLite via await pass-through until swap): departments, services/recordSearch, services/connectors/demo, classify, extract, config, routes/agentRules, routes/auth.

## Remaining
- Convert: routes/staff, services/auth, routes/files, routes/publicChat, routes/requests, server.js.

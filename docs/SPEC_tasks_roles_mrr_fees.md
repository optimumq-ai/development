...
- **The two clocks Kevin wants** (processing vs review) fall out because the author's `redaction` task and the reviewer's `redaction_qa` task are separate tasks, each with its own trail. **DEFERRED refinement:** a distinct `awaiting_review` status at submit (so the author's *processing* clock excludes review-wait) — deferred to an interactively-verifiable follow-up because it restructures the redaction review round-trip + task reconciler. Until then, review timing is clean on the QA task; the author task's processing stretch is coarse (includes review-wait).

...
- **The display — B-core `[BUILT 2026-07-15, Slice B; verify_task_timing 8/8]`.** `taskTiming.js` reads the trail and computes elapsed CALENDAR time per state (in-queue = open+assigned · in-process · in-review = awaiting_review · returned), summed across rounds, current state running to now, plus age-since-submit. `/tasks/mine` carries a `timing` object per task; **My Tasks shows a live clock on each row** ("In queue 7d 5h", "In process 4h", passive "in review" line shows its wait). Raw calendar only — the budgeted-vs-actual overlay is **Slice C**; the per-request **bottleneck breakdown view** (a new visualization stitching `task_events` + `request_history`) is **B-breakdown**, still to build (needs a mockup). Parent roll-up waits for the parent/child model (#11); B computes per-request (= per-child later).

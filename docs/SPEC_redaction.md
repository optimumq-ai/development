...
**Role gate `[BUILT 2026-08-14]`:** every `/api/mass-jobs` MUTATION (create, pause/resume/cancel, run-now, and the 911 connector triggers) requires a redaction permission role (`REDACTION_WORKER`; the orphan `REDACTION_AUTHORITY` also accepted) or a supervising function role (`DIRECTOR`/`SUPERVISOR`; `SYSTEM_ADMIN` always passes). Previously `requireAuth` only — any logged-in staffer could burn redactions across an archive. Reads stay `requireAuth`, consistent with the rest of the processing side.

...
## 2b. Split-canvas v2 intake agent (`/portal/v2`) `[BUILT end-to-end — mobile toggle + cut-over pending]`
...
- **Pending:** mobile step-through toggle `[slice 6]`; cut over `/portal`. Staff-side MRR item-splitting (one child request per described record) is separate — `[[SPEC_tasks_roles_mrr_fees §12]]`, not built; today a multi-record request stores its records in one `description` + flat `selectedRecords`, flagged `is_mrr`.

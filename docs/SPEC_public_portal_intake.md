...
## 2b. Split-canvas v2 intake agent (`/portal/v2`) `[PARTIAL — form + chat + results canvas built; submit pending]`
...
- **Pending:** form→`/public/submit` wiring (persist request + all attached `children`/selected records) `[slice 5]`; mobile step-through toggle `[slice 6]`; cut over `/portal`. Structured-field storage (address, fee-choice, `requestor_type`) already persisted `[slice 1]`.

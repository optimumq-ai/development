...
| F1 | **"What the law lets you charge" — BUILT 2026-08-26** (`/setup/fee-law`, `services/feeLaw.js`, `routes/feeLaw.js`; canvas `docs/mockups/hub_links/fee_law/`, WORKING §2b). The 35 verified fee items are read from the locked state's template file (identical key set in all 32; `parseValue` turns the prose into figures). **State mandate** window (basis fixed/ceiling/floor, 22 in TX): law figure read-only; **ceiling rows carry a city rate that defaults to the ceiling** (Kevin 2026-08-25) and are refused above it; fixed rows "as law"; floor rows start at the minimum; two items flagged "needs requestor ledger". **Deferral** window (13 in TX): a figure, "none" or "actual"; or a fee policy document (text/paste → `feePolicyExtract`) filed as `document` decisions with the extractor's citation as reference. Decisions live in `jurisdiction_rules` domain `fee_schedule_decisions`. **Approve** is refused while a deferral row is undecided, passes the state fee-bounds gate, and writes ONE `fee_profiles` row (FR, version n, active; previous active → superseded, never edited) mapped by each item's engine path, plus a `config_history` row carrying every decision. Hub `fee_law` reader: no jurisdiction → not_started; decisions but no version → in_progress "no fee schedule version yet"; version → in_progress "fee schedule vN · not yet confirmed" → attest → ready. Door `/setup/fee-law`. **Layout (Kevin 2026-08-26): four tabs** — State mandate · City decisions · Fee policy document · **Test an estimate** (the live fee-engine sandbox — since 2026-09-14 the result shown is the requestor's notice, not a computation card, extra costs are a list and the purpose choices are Standard · Commercial; `POST /fee-law/preview` against the approved schedule or the unapproved draft, with "It behaves correctly / Something is off" → `/api/onboarding/fees/test-result`, which the hub's `fee_test` row reads; `fee_test` door → `/setup/fee-law?tab=test`). Every authority citation opens the research record (`/jurisdiction-profile/rules-research/:id`, verbatim statute language). **2026-09-14 (Kevin's 2026-09-13 City-tab markup):** the resolver reads BOTH corpora — `pruned/pruned_discovery.json` (ST-00NN) and the step-2 fee-gap file `alignment/fee_gap_new_rules.json` (ST-9NNN: overhead, media, certification, commercial, bands, cap, repeat), so the Commercial-surcharge and Certified-copy popups now show their statute records; a silent item's popup shows the research note ("What the research found") instead of nothing. **Omitted items (`screen.omitted[]`, `feeLaw.OMIT_RULES`):** an item the state neither sets nor leaves to the city is dropped from the screen with its reason (footnote on the City tab), a decision on it is refused as unknown, the engine keeps the skeleton default. First rule — **Minimum fee** is omitted where the state is silent on it AND its copy charges are a fixed or capped statutory schedule (`dup.bw.rate.resolution` ceiling/value); it stays where the state speaks (OH, TN) or leaves copy charges to the body's own reasonable-cost schedule (a city may adopt a minimum as part of it). TX: omitted (13 → 12 deferral rows). The rule is applied when the template is READ, not baked into the generated templates (never hand-edit generated templates; it can move into `build_state_templates.js` at the next regeneration). `verify_fee_law` 22/22. Not in this slice: PDF text extraction (text/paste only), graduated page bands (were a rate-table edit; **C9, 2026-08-30: the v1 Fee Configuration screen is RETIRED** — it loaded blank and Fee rules is the schedule; `/fee-config` and `/admin?tab=fees` redirect to `/setup/fee-law`; until bands get a Fee rules surface they are reachable only through `PUT /api/fee-profiles/:id`), the SS context. | built |

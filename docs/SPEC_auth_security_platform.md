...
JWT access tokens (8h) carrying id/email/name/department + **function roles and permission roles**; `requireAuth` / `requireRole` middleware gates routes. **Shared write gates `[BUILT 2026-08-14 → 2026-08-18]`** live in `middleware/auth.js` so a bar cannot drift between route files: `requireRedactionWork` (processing surface — SPEC_redaction §6), `requireRequestWork` (a request's files — SPEC_record_search_task_screen §4a), `requireTaxonomyEdit` (SYSTEM_ADMIN/DIRECTOR — SPEC_taxonomy_classification §1); config surfaces (redactionConfig, repositories) use `requireRole('SYSTEM_ADMIN','DIRECTOR')` inline. `SYSTEM_ADMIN` passes every gate; reads stay `requireAuth`. Remaining `requireAuth`-only WRITES worth a pass: `requests.js` per-request acts (`/:id/stage`, `/:id/assign`, clarification, effort, intents) — the request domain scopes LISTS to team/assignee for non-elevated staff but does not scope or role-gate these mutations; `onboarding.js` phase writes. Password hashing: **SHA-256 with a static salt** (the bcrypt-deadlock workaround) — `[HARDENING: upgrade to a proper KDF (bcrypt/argon2) + per-user salts pre-production]`. No MFA `[HARDENING — PRE_RELEASE_HARDENING Category 2]`.

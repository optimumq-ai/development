...
- **Parent/child is BUILT (2026-07-16).** `docs/SPEC_parent_child_lifecycle.md` is the single binding spec (`SPEC_tasks_roles_mrr_fees.md` §12 is a pointer stub). Every request is now a PARENT (citizen, number, money, statutory clock) with 1..n CHILDREN (description, stage, routing, and every FK that hangs off work); a single-record request is n = 1, not a special case. `BUILD_PRIORITY` item 11 is done: wrap (§8), the portal emitting n children, and the queue (§7). **Still open:** the parent/child field-design pass (§4.4) and MRR classification roll-up (§6). The MRR hub is BUILT (BW6/processing-UI, `verify_bw6_mrr`; §14.3's deferral note is history — binding design is `SPEC_processing_ui.md` screen 5). §14.2: as built, children auto-commit through the normal engine with the hub's hand-assigned activities on top; the surviving question (classifier hint on the child assign-picker) is parked with Kevin (Draft 5 §3 residuals).
